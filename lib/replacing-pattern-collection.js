...
import { getActivatePackage, getConfigAndSetOnDidChange } from "./utilities/atom-utility";
...
export default {
  ...
  activate() {
    ...
    this.filePath = getConfigAndSetOnDidChange(`${this.name}.defaultFilePath`, this.subscriptions, this.getFilePath());
    this.read();
  ...
  failed() {
    return (err) => {
      atom.notifications.addWarning("FilePath is not valid file", { "detail": err, "dismissable": true });
    };
  },
  ...
  writeSuccess() {
    return () => {
      atom.notifications.addSuccess("Write to file was successful", { "detail": this.filePath, "dismissable": false });
    };
  },
