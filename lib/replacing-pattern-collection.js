...
export default {
  ...
  activate() {
    ...
    this.findOptions.push(new RPCFindOptions());
    this.getFAndRPackage();

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'replacing-pattern-collection:toggle': () => this.toggle()
    }));
    let nextFunc = this.view.next();
    let lastFunc = this.view.last();
    let previousFunc = this.view.previous();
    let firstFunc = this.view.first();
  ...
  setFindOptions() {
    return (findOptions) => {
      ...
      this.view.setCurrentOption(findOptions[0], 0);
      this.view.setOptionsNumber(this.findOptions.length);
    };
  },
  ...
  getFindOptions() {
    ...
    this.view.setCurrentOption(tempOption, this.findOptions.length - 1);
    this.view.setOptionsNumber(this.findOptions.length);
    this.view.discriptionEditor.view.focus();
  },
