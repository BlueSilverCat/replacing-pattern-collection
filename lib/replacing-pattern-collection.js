...
import { getActivatePackage } from "./utilities/atom-utility";
...
export default {
  ...
  activate() {
    ...
    this.filePath = atom.config.get(`${this.name}.defaultFilePath`);
    this.read();
  ...
  failed() {
    return (err) => {
      atom.notifications.addWarning(`${this.name}: FilePath is not valid file`, { "detail": err, "dismissable": true });
    };
  },
  ...
  getFAndRPackage() {
    if (isEmpty(this.fAndRPackage) === true) {
      ...
      if (isEmpty(this.fAndRPackage) === true) {
        atom.notifications.addError(`${this.name}: Cannot activate find-and-replace.`,
  ...
  writeSuccess() {
    return () => {
      atom.notifications.addSuccess(`${this.name}: Write to file was successful`, { "detail": this.filePath, "dismissable": false });
    };
  },
